...
## Root cause of the persistent “31 open” count

The number is accurate for the current representation, but it is easy to
misread:

- `todo.md` contains 96 checklist rows: 65 checked and 31 unchecked.
...
Therefore, “31 tasks remain” is not the precise statement. The precise
statement is: **31 release-acceptance rows are unchecked; the local
...
- The acceptance matrix is the correct place to see which package-specific
  gates remain.
- The 31-row number will not decrease until either the strict acceptance gates

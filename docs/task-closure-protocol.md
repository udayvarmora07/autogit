...
## Root cause of the persistent “24 open” count

The number is accurate for the current representation, but it is easy to
misread:

- `todo.md` contains 96 checklist rows: 72 checked and 24 unchecked.
...
Therefore, “24 tasks remain” is not the precise statement. The precise
statement is: **24 release-acceptance rows are unchecked; the local
...
4. **Evidence-first reporting.** Every completion message must include the
   ...
   remaining external gate. The headline should say “24 unchecked acceptance
   rows,” not “24 unfinished tasks.”
...
- The acceptance matrix is the correct place to see which package-specific
  gates remain.
- The 24-row number will not decrease until either the strict acceptance gates

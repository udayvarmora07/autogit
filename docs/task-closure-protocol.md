...
## Root cause of the persistent “25 open” count

The number is accurate for the current representation, but it is easy to
misread:

- `todo.md` contains 96 checklist rows: 71 checked and 25 unchecked.
...
Therefore, “25 tasks remain” is not the precise statement. The precise
statement is: **25 release-acceptance rows are unchecked; the local
...
4. **Evidence-first reporting.** Every completion message must include the
   ...
   remaining external gate. The headline should say “25 unchecked acceptance
   rows,” not “25 unfinished tasks.”
...
- The acceptance matrix is the correct place to see which package-specific
  gates remain.
- The 25-row number will not decrease until either the strict acceptance gates

...
| P4-02 | `scripts/check-shell.sh` runs `bash -n` and ShellCheck when available/required. `scripts/shell_scripts_test.go` injects malformed suite, artifact, canary, and benchmark conditions and checks fail-closed behavior. |
...
| P4-08 | `cmd/autogit-evidence` and `scripts/generate-evidence.sh` bind suite results, controls, threat IDs, toolchain, platform, and exact Git HEAD into JSON. Dirty working trees are rejected unless `--allow-dirty` is explicit. |
...
The following checks passed during this implementation slice:

```text
actionlint .github/workflows/*.yml
go test ./internal/provider ./internal/gitport ./internal/compatibility ./scripts
bash scripts/check-shell.sh                    # ShellCheck 0.11.0
bash scripts/check-dependencies.sh
bash scripts/artifact-smoke.sh --binary <built artifact>
...
AUTOGIT_FUZZ_TIME=30s bash scripts/test-suites.sh fuzz
bash scripts/check-shell.sh
bash scripts/check-dependencies.sh
bash scripts/check-compatibility.sh
bash scripts/generate-evidence.sh --output docs/release-evidence/phase-4-quality.json
```

This bundle does not claim that native artifact execution, a live provider
canary, a 100,000-input fuzz quota, signed provenance, or named review has
already occurred. The phase and alpha gates in `todo.md` remain authoritative.

...
var controls = []control{
	{ID: "P1-04", Status: "implemented", Requirements: []string{"FR-VER-003", "NFR-SEC-001"}, Tests: []string{"TestIsolationCapabilitiesAreExplicitAndFailClosed", "TestIsolationCapabilityReportsPlatformFallbacks", "TestTrustedRegistryRecordsAchievedProcessBoundedTier", "TestTrustedEvidenceInvalidatesEveryBindingDimension", "TestRunEnforcesFilesystemAllowlistAndNetworkDenial", "TestRunRejectsNestedUserNamespaceCreation", "TestLandlockABIProbeIsNonMutatingAndExplicit", "TestLandlockDirectEnforcement", "TestWindowsSupervisorTerminatesDescendants"}, Commands: []string{"go test ./internal/process ./internal/verification", "GOOS=windows GOARCH=amd64 go test -c ./internal/process", "GOOS=windows GOARCH=amd64 go test -c ./internal/verification", "GOOS=darwin GOARCH=arm64 go test -c ./internal/process", "GOOS=darwin GOARCH=arm64 go test -c ./internal/verification"}, Threats: []string{"TM-009", "TM-025", "GATE-005"}},
	...
	{ID: "P5-07", Status: "implemented", Requirements: []string{"NFR-SAF-001", "NFR-REL-001"}, Tests: []string{"TestSecurityPolicyDefinesSeverityResponseTargets", "TestReleaseInstallDrillCoversNativeBinaryLifecycle", "TestReleaseRollbackDrillProducesRedactedEvidence"}, Commands: []string{"bash scripts/release-install-drill.sh --previous PATH --candidate PATH --output FILE", "bash scripts/release-rollback-drill.sh --output FILE"}, Threats: []string{"RISK-RELEASE-IDENTITY", "GATE-006"}},
}

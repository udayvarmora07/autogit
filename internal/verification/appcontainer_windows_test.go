...
func TestTrustedVerificationRecordsWindowsAppContainerAttestation(t *testing.T) {
	...
	request := TrustedRequest{CandidateDigest: digest('a'), BaseDigest: digest('b'), PolicyDigest: digest('c'), GuardDigest: digest('d'), Dir: filepath.Clean(work), OverallTimeout: 60 * time.Second}
